...
impl Boards {
    ...
    where
        I: Into<String>,
    {
        self.jira.get("agile", &format!("/board/{}", id.into())).await
    }
    ...
    pub async fn list(&self, options: &SearchOptions) -> Result<BoardResults> {
        ...
        self.jira.get::<BoardResults>("agile", path.join("?").as_ref()).await
    }
    ...
    // See the [jira docs](https://docs.atlassian.com/jira-software/REST/latest/#agile/1.0/board-getAllBoards)
    // for more information
    // pub async fn iter<'a>(&self, options: &'a SearchOptions) -> Result<BoardsIter<'a>> {
